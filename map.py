...
# DATE CREATED: 7 Nov 2022
# REVISED DATE:
# PURPOSE: implement a virtual self-driving car that can learn to
#          navigate through a given environment using reinforcement learning
#          The code uses the Kivy library for creating a graphical user interface and the Dqn object from aii.py,
#          which contains the neural network that represents the Q-function used in the Q-learning algorithm
#          The virtual car is equipped with three sensors that detect the presence of sand on the map,
#          and the car receives a reward based on its distance from the goal position and its speed.
#          The purpose of the code is to train the car to reach the goal position while avoiding obstacles
#          and minimizing its travel time.
